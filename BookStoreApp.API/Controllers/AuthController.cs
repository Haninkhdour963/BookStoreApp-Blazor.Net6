...
namespace BookStoreApp.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            logger.LogInformation($"Registeration Attempt for {userDto.Email}");
            try
            {
        ...
        public async Task<ActionResult<AuthResponse>> Login(LoginUserDto userDto)
        {
            ...
            try
            {
                ...
                var passowrdValid = await userManager.CheckPasswordAsync(user, userDto.Password);

                if (user == null || passowrdValid==false)
                { 
        ...
        {
            var secuityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(configuration["JwtSettings:Key"]));
